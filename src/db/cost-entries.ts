...
// (dev/staging → play), AWS, Cloudflare, manual entries, etc.
//
...
/**
 * Aggregate image-gen cost entries by environment (for the dashboard breakdown).
 * Only counts remote reports (service = 'image_gen'); the local server's own
 * generation_log costs are merged in by the caller under its own environment.
 */
